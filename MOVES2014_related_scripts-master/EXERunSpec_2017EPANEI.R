...
DS <- read_excel(file_DS,sheet = "RunSpec")
...
DS[is.na(DS)] <- "-"
checkfile <- sprintf("%s/RunSpecs/ExeRunSpec_2017_EPANEI.txt",path_DS)
if (file.exists(checkfile)) file.remove(checkfile)
write("@echo off",checkfile,append = TRUE)
write("echo Changing to the MOVES folder and compiling code...",checkfile,append = TRUE)
...
for (c in 1:length(DS$countyID)){
  XMLfile <- sprintf("%s/%s",gsub("\\\\","/",DS$Directory[c]),DS$`Run Spec Filename`[c])

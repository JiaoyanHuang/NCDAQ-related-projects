...
DS[is.na(DS)] <- "-"
checkfile <- sprintf("%s/XML_Importers/FileExistCheck.txt",path_DS)
...
for (c in 1:length(DS$countyID)){
  ...
  line <- sprintf("REM %s",XMLfile)
  write(line,checkfile,append = TRUE)
  XML <-  readLines(XMLfile)
  for (l in 1:length(linenumber)){
    file_tmp <- strsplit(XML[linenumber[l]],split = "[<|,>]")[[1]][3]
    line <- sprintf("if not exist %s echo Missing %s",file_tmp, file_tmp)
    write(line,checkfile,append = TRUE)
  }
  if(DS$`I&M Frequency`[c] == "annual"){
    file_tmp <- strsplit(XML[198],split = "[<|,>]")[[1]][3]
    line <- sprintf("if not exist %s echo Missing %s",file_tmp, file_tmp)
    write(line,checkfile,append = TRUE)
  }
}

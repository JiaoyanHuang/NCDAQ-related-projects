...
# words
###USER INPUTS###
iem.wd <- "C:/Users/jhuang/Desktop/TEST/ASOS_raw" #download location
date1 <- ISOdate(2017,1,1) #start date in year, month, day format
date2 <- ISOdate(2018,1,1) #end date in year, month, day format
user.network <- c("ASOS")
user.state <- c("NC") #state
user.faaid <- c("1A5") #site FAA identifier - leave empty and a list will print for your reference
#################
...
data_tmp <- read.csv(str_c(outfn,".txt"),skip =5)
data <- data_tmp[,c(1:5,7)]
write.csv(data,str_c(outfn,".txt"),row.names = F)
